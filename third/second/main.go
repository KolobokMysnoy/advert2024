...
func isDo(from int, runes []rune) (bool, bool, int) {
	if from+3 >= len(runes) {
		return false, false, from
	}

	if !(runes[from] == 'd' && runes[from+1] == 'o') {
		return false, false, from
	}

	if runes[from+2] == '(' && runes[from+3] == ')' {
		return true, true, from + 1
	}

	if from+6 >= len(runes) {
		return false, false, from + 1
	}
	...
	if runes[from+2] == 'n' &&
		...
		runes[from+5] == '(' &&
		runes[from+6] == ')' {
		return true, false, from + 6
	}

	return false, false, from + 1
}

func searchMul(from int, runes []rune, canDo bool) ([]int, bool) {
	i := from
	...
	for i < len(runes) {
		if runes[i] == 'd' {
			ok, newState, newInd := isDo(i, runes)
	...
	return retNumbs, canDo
}

func getLineSum(str string, canDo bool) (int, bool) {
	runes := []rune(str)
	numbs, newCanDo := searchMul(0, runes, canDo)
	canDo = newCanDo
...
func main() {
	...
	canDo := true
	for i := range readCh {
		lineSum, newState := getLineSum(i, canDo)
