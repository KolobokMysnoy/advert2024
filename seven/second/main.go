...
// ----
func concatNumbs(first, second int) (int, error) {
	convertedFirst := strconv.Itoa(first)
	convertedSecond := strconv.Itoa(second)

	resultConverted := convertedFirst + convertedSecond
	return strconv.Atoi(resultConverted)
}
...
func checkOperators(index int, currentSum, neededSum int, numbers []int, operator string) []int {
	...
	case "||":
		var err error
		localSum, err = concatNumbs(currentSum, rightValue)
		if err != nil {
			fmt.Println(fmt.Errorf("error at concatNumbs: %w", err))
			return []int{}
		}
	}
	...
	concatSum := checkOperators(index+1, localSum, neededSum, numbers, "||")

	sums = append(sums, plusSum...)
	sums = append(sums, productSum...)
	sums = append(sums, concatSum...)
...
func checkLine(inputLine string) (int, bool) {
	...
	concatSums := checkOperators(0, numbers[0], neededValue, numbers, "||")
	for _, v := range concatSums {
		if v == neededValue {
			return neededValue, true
		}
	}
